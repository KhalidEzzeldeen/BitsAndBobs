...
% for ii = 1:ite
    ...
    for jj = 1:length(tempIndex)
        ...
        if isempty(find(CurSolVec==tempz,1))
            Loc(jj) = 0;
            dH(jj) = sum(A(tempz,setdiff(1:N,CurSolVec))) -...
                     sum(A(tempz,CurSolVec));
        else
            Loc(jj) = 1;
            dH(jj) =  sum(A(tempz,CurSolVec)) - ...
                      sum(A(tempz,setdiff(1:N,CurSolVec)));
        end
    end
    %Update with best, remove Tabued, etc
    z = tempIndex(dH==max(dH));
    if isempty(TabuList) || sum(ismember(z,TabuList(1,:)))==0
            ...
            if ~isempty(tempIndex)
                ...
                if ~isempty(tempIndex)
                    ...
                    if isempty(find(CurSolVec==z,1))
                        Loc = 0;
                        dH = sum(A(z,setdiff(1:N,CurSolVec))) -...
                             sum(A(z,CurSolVec));
                    else
                        Loc = 1;
                        dH =  sum(A(z,CurSolVec)) -...
                              sum(A(z,setdiff(1:N,CurSolVec)));
                    end
                end
            end
