    ...
    def makeWidgets(self, numrow, numcol):
        ...
        for i in range(numrow):
            ...
            for j in range(numcol):
                ent = Entry(self, relief=RIDGE, width=20)
...
if __name__ == '__main__':
    import sys
    root = Tk()
    canv = Canvas(root, relief=SUNKEN, bg='white')
...
##    root.title('Summer Grid')
    ...
    if len(sys.argv) != 3:
        gridded = SumGrid(canv)    # .grid() works here too
    else:
        rows, cols = eval(sys.argv[1]), eval(sys.argv[2])
        gridded = SumGrid(canv, rows, cols)
    gridded.pack()

    # vert scroll bar, links
    vsbar = Scrollbar(root)
    vsbar.config(command=canv.yview)
    canv.config(yscrollcommand=vsbar.set)
    vsbar.pack(side=RIGHT, fill=Y)
    # horz scroll bar, links
    hsbar = Scrollbar(root, orient='horizontal')
    hsbar.config(command=canv.xview)
    canv.config(xscrollcommand=hsbar.set)
    hsbar.pack(side=BOTTOM, fill=X)

    # ad-hoc method for getting correct dimensions, placement;
    # seems that 'window' positioning is from center of object...
    # also, this particular setting is for grid in question; need gen
    # text / grid / button to pixel conversion, eh?
    canv.config(scrollregion=(0,0,
                              gridded.numcol*20*3.8*2, (gridded.numrow+3)*12*2))
    canv.pack(expand=YES, fill=BOTH)
    canv.create_window(gridded.numcol*20*3.8, (gridded.numrow+3)*12,
                       window=gridded)
    mainloop()
