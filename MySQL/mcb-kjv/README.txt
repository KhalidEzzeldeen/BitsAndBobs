...
load_kjv.sql - SQL script to load kjv.txt into kjv.sql
index_kjv.sql - SQL script to add indexes to the kjv table (use after
...
% mysql cookbook < index_kjv.sql

If you want to use the kjv.pl search script, you'll also need to create
...
- The Cookbook includes book names (Genesis, Exodus, etc.) as well as
  the numbers.  This makes the dataset non-normal, but avoids the
  need for a join to display book names in search results.
