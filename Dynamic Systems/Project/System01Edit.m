...
%     end
    ...
    [~,uSolAct] = ode45(@(t,u)SysChangeAct(t,u,UpdateThresh,NumSec,PeakTime,Type),...
...
function du = SysChangeAct(t,u,UpdateThresh,NumSec,PeakTime,Type)
...
function J = ControlCostCalc(CNow,tCur,uCur,NumSec,PeakTime,Type)
    ...
    [~,uSolCon] = ode45(@(t,u)SysChangeCont(t,u,CNow,tCur,NumSec,PeakTime,Type),...
                        [tCur,T],uCur);
...
function du = SysChangeCont(t,uNow,CNow,tCur,NumSec,PeakTime,Type)
    global A TFunc
...
function CFunc = ControlEffect(t,uNow,CNow,tCur,NumSec,Type)
    ...
    switch Type
        case 0          %Constant peicewise control
            ...
            CFunc = (C(end-1)-uNow).*(C(end)*C(1:end-2));
...
function dJ = CostCalc(t,~,CNow,tCur,NumSec,Type,Where)
    ...
    global R TFunc CurrTemps AllCT
    if isempty(CNow)
        CurrControlStrategy = find(AllCT(1,:)<=t,'last');
        tStart = AllCT(1,CurrControlStrategy);
        CNow = AllCT(2:end,CurrControlStrategy);
    ...
    eff = C(end-1)/abs(C(end-1)-CurrTemp);
    dJ = diag([eff,1])*C(end-1:end)'*diag(R)*C(end-1:end)*diag([eff,1])';
end
